...
EXTRACTOR_SAMPLE_LIMIT  = 3      # 只取前 N 条跑 LLM，节省测试时间（None=全量）
...
def main():
    ...
    extractor = Extractor(
        db_conn=raw_conn,
        auto_insert_new_type=not EXTRACTOR_DRY_RUN,
    )
    ...
    _assert(all(i.device_introduce for i in enriched),             "device_introduce 为空")
    ...
    for idx, item in enumerate(enriched, 1):
        ...
        if item.dim3_suggest_name:
            new_type_count += 1
            logger.warning(
                f"           ⚠ 新三级方向建议   : [{item.dim3_suggest_style_id}] "
                f"→ \"{item.dim3_suggest_name}\""
                + ("（已自动入库）" if item.device_type_id else "（待人工审核）")
            )
    ...
    if new_type_count:
        logger.warning(f"      共发现 {new_type_count} 条新三级方向建议，请检查上方 ⚠ 日志")
